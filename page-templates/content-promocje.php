<div class="page album">
	<div class="wrapper">
		<h1 class="wnl-page-title no-shadow">
			Jak skorzystać ze zniżek?
		</h1>
		<div class="cards">
			<div class="card">
				<div class="strong card-content">
					<p>W trosce o przyszłość kolejnych edycji kursu promocje nie łączą się. 😉</p>
					<div>Zniżki możesz wykorzystać nawet po opłaceniu zamówienia, ale tylko do oficjalnego końca zapisów!</div>
				</div>
			</div>
		</div>
		<div class="cards">
			<div class="card alignleft">
				<h4 class="card-title">
					50% na kolejne edycje Kursu Internetowego
				</h4>
				<div class="card-content">
					<ol>
						<li><a href="https://platforma.wiecejnizlek.pl" target="_blank">Zaloguj się na platformie</a>.</li>
						<li>Przejdź na <a href="https://wiecejnizlek.pl/zapisy" target="_blank">wiecejnizlek.pl/zapisy</a>.</li>
						<li>Wybierz Kurs Internetowy - zniżka zostanie naliczona automatycznie, o czym poinformuje Cię niebieskie powiadomienie widoczne na górze strony. 🙂</li>
					</ol>
				</div>
			</div>
		</div>
		<div class="cards">
			<div class="card alignleft">
				<h4 class="card-title">
					Study Buddy - po 100zł dla Ciebie i znajomej osoby
				</h4>
				<div class="card-content">
					<ol>
						<li>Zakup kurs poprzez <a href="https://wiecejnizlek.pl/zapisy" target="_blank">wiecejnizlek.pl/zapisy</a> nie korzystając z żadnych innych zniżek!</li>
						<li>Po zakończeniu płatności, na platformie w zakładce <a href="https://platforma.wiecejnizlek.pl/app/myself/orders" target="_blank">KONTO > Twoje zamówienia</a> znajdziesz kod Study Buddy, który możesz wysłać znajomej osobie.</li>
						<li>Gdy Twoja znajoma osoba zakupi kurs wykorzystując Twój kupon, a my odnotujemy wpłatę - do Twojego zamówienia zostanie naliczony rabat 100zł.</li>
						<li>Jeżeli Twoja znajoma zakupiła już kurs - nic straconego! Jeżeli nie użyła innych zniżek, wciąż może użyć Twojego kodu Study Buddy w zakładce <a href="https://platforma.wiecejnizlek.pl/app/myself/orders" target="_blank">KONTO > Twoje zamówienia</a> klikając na Dodaj lub zmień kupon.</li>
						<li>Zwrot możesz wypłacić na konto, z którego opłacony był kurs. Przycisk znajdziesz w zakładce <a href="https://platforma.wiecejnizlek.pl/app/myself/orders" target="_blank">KONTO > Twoje zamówienia</a>.</li>
					</ol>
				</div>
			</div>
		</div>
		<div class="cards">
			<div class="card alignleft">
				<h4 class="card-title">
					Study Group - 10% dla całej grupy
				</h4>
				<div class="card-content">
					<ol>
						<li>Zbierz adresy e-mail od grupy od 10 do 30 zainteresowanych kursem osób</li>
						<li>Wyślij listę adresów na info@wiecejnizlek.pl</li>
						<li>W ciągu 5 dni roboczych wszystkie osoby otrzymają od nas maila ze swoim unikalnym kodem.</li>
						<li>Otrzymany kod można zrealizować wchodząc na <a href="https://wiecejnizlek.pl/voucher" target="_blank">wiecejnizlek.pl/voucher</a>.</li>
						<li></li>
					</ol>
				</div>
			</div>
		</div>

		<?php get_template_part( 'page-templates/content', 'recirculation' ); ?>
	</div>
</div>
